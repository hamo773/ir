...
class MA_PS_DQN(nn.Module):
    def __init__(self, num_agents, input_size, output_size):
        ...
        self.q_values = nn.ModuleList([
            nn.Linear(self.hidden_dim , output_size) for _ in range(num_agents)
        ])
...
class Agent:
    def __init__(self, num_agents_, state_dim, action_dim):
        ...
        self.dqn_model = MA_PS_DQN(num_agents_,state_dim,action_dim)
        self.dqn_target = MA_PS_DQN(num_agents_,state_dim,action_dim)  
