...
class DQN(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc3 = nn.Linear(self.hidden_dim, output_size)
...
class Agent:
    def __init__(self, num_agents_, state_dim, action_dim):
        ...
        for model in self.dqn_model:
            model.to(self.device)

        for target_model in self.dqn_target:
    ...
    def save_model(self, SIZE):
        ...
        for i in range(1, SIZE):
            for j in range(1, SIZE):
                if i != j:
                    torch.save(self.dqn_model[k], "model/dqn_model/{}_{}".format(i, j))
                    k = k + 1
    ...
    def load_model(self, SIZE):
        ...
        for i in range(1, SIZE):
            for j in range(1, SIZE):
                if i != j:
                    self.dqn_model[k] = torch.load("model/dqn_model/{}_{}".format(i, j), map_location=self.device)
                    #print(i,j)
                    k = k + 1
    def update(self):
