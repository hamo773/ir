...
def DRL_thread():
    ...
    reward_memory = np.zeros((SIZE,SIZE))
    ...
    while True:
        ...
        for i in range(1,SIZE):
            ...
            for j in range(1,SIZE):
                if i != j:
                    ...
                    reward = all_reward[str(i)][str(j)][action]
                    reward_all += reward
                    reward_bwd = all_reward_indicator[str(i)][str(j)][action][0]
                    reward_all_bwd += reward_bwd
                    reward_delay = all_reward_indicator[str(i)][str(j)][action][1]
                    reward_all_delay += reward_delay
                    reward_loss = all_reward_indicator[str(i)][str(j)][action][2]
                    reward_all_loss += reward_loss
                    #print(action)
                    #onehot = np.zeros(20)
                    #onehot[action] = 1
        ...
        if step != 1:
            #maddpg_agents.append_sample(state_memory,action_memory,mean_actions_memory, state,all_agent_reward_list)
            reward_memory_=calc_cluster_reward(reward_memory)
            #print(reward_memory[0],reward_memory_[0])
            print(state_memory,state)
            madqn_agents.append_sample(state_memory,action_memory, state,reward_memory_)   ## 為了next state 還是要下一step再存
        ...
        #print(np.shape(np.mean(all_agent_action_list,axis=0)))
        action_memory = all_agent_action_list 
        reward_memory = all_agent_reward_list
        #mean_actions_memory = np.mean(all_agent_action_list,axis=0)
...
def calc_cluster_reward(reward):
    num_agents=(SIZE-1)*(SIZE-2)
    #cluster_count=[0] * cluster_number
    total_reward=0
    reward_ = [0]*num_agents
    for i in range(num_agents):
        total_reward += reward[i]

    average_reward=total_reward/num_agents
    for i in range(num_agents):
        reward_[i] += reward_a*reward[i] + reward_c*average_reward
    return reward_
